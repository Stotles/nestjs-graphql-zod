type Suffixed<Suffix extends string, S extends string> = S extends `${string}${Suffix}`
  ? S
  : `${S}${Suffix}`

/**
 * Returns a function which will take a string and return another string with the given suffix, if
 * the suffix was already there then this function will simply return the string value.
 *
 * @typedef S The suffix string.
 * @param {S} suffix The suffix string.
 * @returns {(input: string) => string} The suffixed string.
 * @export
 */
export function withSuffix<S extends string>(suffix: S) {
   ...
   * @returns {Suffixed<S, I>} The suffixed string.
   */
