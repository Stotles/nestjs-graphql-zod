...
describe('QueryOptions / MutationOptions / SubscriptionOptions', () => {
  describe('shared BaseTypeOptions surface', () => {
    ...
    it('should preserve the BaseTypeOptions discriminated union', () => {
      // The whole point of this PR: the upstream `nullable: true | (false |
      // NullableList)` union must survive the wrapper. Extract<> isolates
      // each arm — if the union was flattened (e.g. by an
      // `interface X extends Omit<Union, never>` workaround), TS would
      // collapse both arms into a single object type and the extracted
      // `nullable` literal would no longer narrow to a single value.
