  ...
  describe.each([
    ...
    describe('arrays', () => {
      ...
      it('should handle optional array', () => {
        const info = getFieldInfoFromZod('tags', z.array(z.string()).optional(), defaultOptions, direction)
        expect(info.isOfArray).toBe(true)
        expect(info.isOptional).toBe(true)
      })

      it('should detect nullable items in array', () => {
        const info = getFieldInfoFromZod('tags', z.array(z.string().nullable()), defaultOptions, direction)
        expect(info.isOfArray).toBe(true)
    ...
    describe('enums', () => {
      it('should handle ZodEnum', () => {
        const info = getFieldInfoFromZod('status', z.enum(['active', 'inactive']), defaultOptions, direction)
    ...
    describe('defaults', () => {
      it('should unwrap ZodDefault', () => {
        const info = getFieldInfoFromZod('name', z.string().default('hello'), defaultOptions, direction)
        expect(info.type).toBe(String)
      })
    })
    ...
    describe('zod v4 wrappers', () => {
      ...
      it('should unwrap ZodPrefault', () => {
        const info = getFieldInfoFromZod('name', z.string().prefault('hi'), defaultOptions, direction)
        expect(info.type).toBe(String)
        expect(info.isOptional).toBe(false)
      })

      it('should unwrap ZodLazy', () => {
        const info = getFieldInfoFromZod('name', z.lazy(() => z.string()), defaultOptions, direction)
        expect(info.type).toBe(String)
      })

      it('should resolve ZodLazy that returns a number', () => {
        const info = getFieldInfoFromZod('count', z.lazy(() => z.number()), defaultOptions, direction)
        expect(info.type).toBe(Number)
      })
      ...
      it('should throw when ZodLazy getter is not a function', () => {
        ...
        expect(() => getFieldInfoFromZod('name', broken, defaultOptions, direction))
      ...
      it('should follow Optional through ZodReadonly', () => {
        const info = getFieldInfoFromZod('name', z.string().readonly().optional(), defaultOptions, direction)
        expect(info.type).toBe(String)
        expect(info.isOptional).toBe(true)
      })

      it('should follow Nullable through ZodPrefault', () => {
        const info = getFieldInfoFromZod('name', z.string().prefault('hi').nullable(), defaultOptions, direction)
        expect(info.type).toBe(String)
        expect(info.isNullable).toBe(true)
      })
    ...
    describe('input direction', () => {
      describe('nested objects', () => {
        it('should throw a nested ZodObject without a getDecorator', () => {
          ...
          expect(() => getFieldInfoFromZod('address', inner, defaultOptions, 'input'))
            .toThrow(/no `getDecorator` was provided/)
        })
      })
      ...
      describe('pipes and transforms', () => {
        ...
        it('should throw z.preprocess() with an inner schema', () => {
          ...
          expect(() => getFieldInfoFromZod('status', schema, defaultOptions, 'input'))
    ...
    describe('output direction', () => {
      ...
      describe('pipes and transforms', () => {
        it('should throw a bare .transform()', () => {
          ...
          const schema = z.string().transform(val => val.toUpperCase())
          expect(() => getFieldInfoFromZod('name', schema, defaultOptions, 'output'))
        ...
        it('should throw an error when a string is piped into a z.transform()', () => {
          const schema = z.string().pipe(z.transform(val => val.length))
          expect(() => getFieldInfoFromZod('length', schema, defaultOptions, 'output'))
            .toThrow(/output side of this ZodPipe is a ZodTransform/)
        })
